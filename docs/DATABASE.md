...
- Начальная миграция Sequelize: `server/database/migrations/20260410150000-initial-schema.cjs` (выполняет SQL из `init-schema.sql`).
- Последующие `.cjs` миграции в `server/database/migrations/` фиксируют изменения схемы после initial baseline; применять их нужно в порядке timestamp через `npm run db:migrate -w server`.
...
- Уникальность: `users.email`, пары в `enrollments`, `favorites`, `course_reviews`; уникальные `certificates.enrollment_id` и `certificates.document_number`.
...
- **`enrollments.progress`** — хранимый процент по бизнес-правилам (пересчёт по лучшим баллам за упражнения).
