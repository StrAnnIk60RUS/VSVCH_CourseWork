...
Данный репозиторий содержит всё необходимое для локального запуска и проверки работы приложения (исходный код, `package-lock.json`, SQL-дамп, образец `server/env.example`). Полный текст **раздела 4 «Руководство системного программиста»** приведён в пояснительной записке; здесь — краткая инструкция для проверяющего.

**Репозиторий:** https://github.com/StrAnnIk60RUS/VSVCH_CourseWork — ветка **`main`**, кнопка **Code → Download ZIP**.
...
## Чеклист для автора перед сдачей

| Шаг | Действие |
|-----|----------|
| 1 | Сделать скриншот `server/.env` для **рисунка 4.1** в пояснительной записке (пароль можно скрыть) |
| 2 | На GitHub: репозиторий **Public**, ветка по умолчанию **`main`**, в About — краткое описание проекта |
| 3 | Скачать ZIP с GitHub в **новую папку** и пройти шаги установки только по этому README |
| 4 | Вставить текст раздела 4 из локального файла `docs/4-rukovodstvo-sistemnogo-programmista.md` в Word (файл в git не входит) |
| 5 | Сообщить преподавателю демо-логины из раздела «Тестовые учётные записи» выше |
