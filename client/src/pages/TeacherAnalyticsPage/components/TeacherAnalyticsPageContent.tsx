...
import { formatShortDay } from '../../../utils/dateTime';
...
export function TeacherAnalyticsPageContent() {
  const t = useI18n();
  ...
  const chartSeriesLabels: Record<'submissions' | 'activeStudents', string> = {
    submissions: t.teacherAnalytics.submissionsSeries,
    activeStudents: t.teacherAnalytics.activeStudentsSeries,
  };

  return (
    <PageShell title={t.teacherAnalytics.pageTitle} description={t.teacherAnalytics.pageDescription}>
      <div className="teacher-analytics-page__stack">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    setStatus(
                      response.message ??
                        (response.sent ? t.progress.emailSent : t.progress.demoMode),
                    );
                  } catch (err) {
                    setStatus(getApiError(err));
                  } finally {
                ...
                disabled={!courseId || !userEmail || sendingEmail}
              >
                {sendingEmail ? `${t.teacherAnalytics.sendEmail}...` : t.teacherAnalytics.sendEmail}
              </button>
            </div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <SectionCard title={`${t.teacherAnalytics.activePeriod} (${period})`}>
        ...
        <SectionCard title={t.teacherAnalytics.activityTrend}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={analytics.timeline}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value, key) => [
                      value,
                      chartSeriesLabels[key as 'submissions' | 'activeStudents'] ?? String(key),
                    ]}
                  />
                  <Legend
                    formatter={(value: string) =>
                      chartSeriesLabels[value as 'submissions' | 'activeStudents'] ?? String(value)
                    }
                  />
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={distributionData}>
                    ...
                    <XAxis dataKey="label" />
                    <YAxis allowDecimals={false} />
                    <Tooltip formatter={(value) => [value, t.teacherAnalytics.students]} />
                    <Bar dataKey="value">
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={studentsProgressData} layout="vertical" margin={{ left: 16, right: 16 }}>
                    ...
                    <Tooltip formatter={(value) => [`${value}%`, t.teacherAnalytics.avgProgress]} />
